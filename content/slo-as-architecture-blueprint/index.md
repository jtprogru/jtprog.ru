...
- **Golden Signals** (Google SRE) — `Latency, Traffic, Errors, Saturation`. Универсальный набор для пользовательских сервисов.
- **RED** (Tom Wilkie, Weaveworks) — `Rate, Errors, Duration`. Подмножество Golden Signals, ориентированное на запрос-ответные сервисы.
- **USE** (Brendan Gregg) — `Utilization, Saturation, Errors`. Для ресурсов: CPU, диск, память, сеть.
...
> **Продакт:** «Тогда выкатим за окно 30 дней + 5, чтобы дождаться сдвига окна и обновления бюджета. До BF успеваем».
...
**Правильно:** 4–8 SLI на сервис, привязанных к ключевым шагам пути. Если хочется больше — разбей сервис. Развёрнуто — в Шаге 3, блок «Конкретные SLI для нашего кейса».
