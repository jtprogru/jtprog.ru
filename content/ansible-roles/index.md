...
- Role packages was created successfully
```

В данном примере создается роль под названием `packages` – директория с именем `packages` будет создана в текущей директории откуда запускалась `ansible-galaxy`. Вот так выглядит структура файлов и директорий:
...
Немного о том, что и за что отвечает я тебе рассказал. Теперь я тебе покажу свой подход к разработке ролей, как я стараюсь делать и как я не делаю, а главное поясню "почему так, а не иначе".
...
- Создал директорию;
- Накидал таски/хендлеры/дефолтсы;
- Положил куда требуется;

Если речь идет о публичной роли – той, которую я буду публиковать в Ansible Galaxy – то тут еще проще:
...
Альтернативный вариант которым я пользуюсь чуточку чаще - инициализация директории с ролью через Molecule:
...
У меня была попытка сделать шаблон роли – [sample-role](https://github.com/jtprogru/sample-role). Чтоб можно было форкнуть/скачать архив/скопипастить и иметь уже готовый скелет с тестами в виде Molecule и прочим. Но пока нет времени заняться и собрать из нее нормальный шаблон для [Cookiecutter](https://cookiecutter.readthedocs.io/en/stable/) – если будет желание, кидай PR.
