...
**Self-bootstrap, когда control-зависимости отвалились.** Кэшированные сертификаты и токены, emergency-конфиги, offline-режим для критичных путей. Система поднимается сама. Или хотя бы дотягивает уже идущий трафик, пока её control plane лежит и поднять его некому. Для облака это значит: приватный кластер переживает падение IAM или API провайдера.
...
DIS — это про связность и дисциплину. Архитектура режет blast radius на домены отказа, SLO превращает обещания в измеримый контракт. Нагрузка и хаос по регламенту проверяют гипотезы надёжности раньше, чем это за тебя сделает продакшен. Автоматика работает под kill-switch и bounds. У онколла есть лимит на шум алертов и право остановить поставку. А экономика в конце считает, во сколько всё это обошлось. По отдельности ни один элемент иммунитета не даёт: он появляется там, где они замкнулись в петлю обучения.
...
Порядок, в котором эти элементы вообще имеет смысл строить, — отдельный разговор: пока нет мониторинга, обсуждать автоматическое восстановление рано. Я развернул его в [иерархию надёжности](https://jtprogru.github.io/The-Way-of-SRE/reliability-hierarchy/), а проверку контролов безопасности тем же методом хаос-экспериментов — в [Security Chaos Engineering](https://jtprogru.github.io/The-Way-of-SRE/practices/security-chaos-engineering/).

Хочешь понять, где стоишь — пройдись по пяти маркерам и посчитай, сколько выполняется на самом деле. Картина получится честнее, чем от аудита инструментов или примерки чужих лестниц из предыдущей секции: инструменты и уровни идут следом за связностью, а не наоборот.
