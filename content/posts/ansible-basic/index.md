...
Привет, `%username%`! На страницах этого блога уже [было](/posts/notes-20200925/) небольшое упоминание [Ansible](https://www.ansible.com) – инструмента для управления конфигурацией. Пройдемся по основам, определениям дабы зафиксировать.

> 🔄 **Обновлено 2026-05-15**: проставил актуальные требования к Python, поправил пакет (`ansible` → `ansible-core` после split'а в 2.10) и в конец добавил блок «Что изменилось к 2026» — про коллекции, `ansible-lint`, `molecule` и место Ansible рядом с Terraform/Pulumi.
...
Раньше модули жили монолитом, и одна оплошность в каком-нибудь `cloud.aws.s3_bucket` блокировала релиз всего ansible. Сейчас всё распределено по [коллекциям](https://docs.ansible.com/ansible/latest/collections_guide/index.html) — самодостаточным пакетам модулей и плагинов, которые версионируются отдельно. Ставятся через:
...
- **[ansible-lint](https://ansible.readthedocs.io/projects/lint/)** — статический анализатор. Ловит deprecated-синтаксис, нарушения best practices, прогоняется в pre-commit и в CI.
- **[molecule](https://ansible.readthedocs.io/projects/molecule/)** — тест-фреймворк для ролей. Поднимает контейнеры/виртуалки, прогоняет плейбук, проверяет идемпотентность, валидирует результат через assert'ы. Без него — у тебя нет уверенности, что роль ещё работает.
...
Следующий шаг — разложить плейбуки по [ролям и обвязать их Molecule](/posts/ansible-roles/). И одна история из практики о том, как не надо: [заметка на полях про `ansible.cfg` и ssh](/posts/notes-20200925/). А если нужен не инструмент, а рамка — где заканчивается управление конфигурацией и начинается [инфраструктура как код](https://jtprogru.github.io/The-Way-of-SRE/engineering/infrastructure-as-code/), — про это есть отдельный лист в карте компетенций.
