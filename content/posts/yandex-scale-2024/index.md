...
- Всегда есть шанс поломать не только свои сервисы, но и саму хаос-платформу, поэтому ран-буки по ее восстановлению необходимо держать при себе;
...
- Chaos Engineering позволяет повысить надежность и отказоустойчивость системы, приблизить к [SLO](/posts/slo-as-architecture-blueprint/), улучшить взаимодействие и навыки команды, а так же сократить MTTR.

Хаос-эксперименты сами по себе — только один из столпов того, что Gartner называет [Digital Immune System](/posts/digital-immune-system/): рядом должны стоять observability и автоматическое восстановление, иначе ты просто ломаешь прод без выводов.

### Как подключаться к S3 из VPC без доступа к интернету
...
Невозможно обойти стороной тему безопасности при разговоре про публичные облака. Поэтому немного о том, что подсветили ребята из Yandex Cloud:
