...
Привет, `%username%`! В прошлой статье — [«SLO как чертёж архитектуры»](/posts/slo-as-architecture-blueprint/) — я закончил тезисом, который тут же хочется развить: SLO выбирается не инженерами в одиночку, а вместе с бизнесом, потому что число в нём привязано к деньгам. И вот тут начинается самое интересное. Потому что «вместе с бизнесом» в реальности чаще всего выглядит так: SRE придумал SLO, нарисовал дашборд, показал на демо, продакт покивал — и все разошлись. Чертёж готов. Надёжность... не выросла.
...
Берём наш маркетплейс. Из «чертежа»: GMV порядка $10M в день, поиск кормит 40–70% покупок, каждые `+100 ms` латентности на ключевом read path стоят примерно `−0.5…1.5%` конверсии. Отсюда строится простой, но мощный мост:
...
Посчитаем на пальцах для одного и того же 15-минутного инцидента в поиске — полная недоступность пути. GMV $10M в день размазан неравномерно: обычным днём через поиск проходит порядка $4M выручки (предположим, 40% покупок стартуют с поиска), на пике распродажи дневной GMV умножается в `×3–5`, и доля стартующих с поиска покупок растёт.

| | Обычный вторник | Пик распродажи |
|---|---|---|
| Выручка через поиск за день | ~$4M | ~$15M |
| Выручка через поиск в час (грубо) | ~$170k | ~$1.2M |
| Полный простой поиска 15 минут | ~$42k | ~$300k |
...
- **Количество инцидентов в абсолюте.** Стимулирует молчать о мелких. Если команду меряют числом инцидентов, она перестаёт их заводить — и ты теряешь сигнал. Это классический Goodhart: метрика, ставшая целью, перестаёт быть метрикой.
...
### Продуктовая надёжность как KPI команды — осторожно с Goodhart

Здесь подстерегает ловушка, ради которой стоит притормозить. Заманчиво объявить «продуктовую надёжность KPI команды SRE» и мерить команду по SLO продуктов. Не делай этого напрямую — это прямой путь к токсичным SLO.

Логика Goodhart: как только SLO становится KPI, по которому оценивают команду, команда начинает оптимизировать SLO, а не надёжность. Появляются заниженные таргеты (чтобы всегда выполнять), исключённые из знаменателя «неудобные» события, споры о том, считать ли инцидент инцидентом. Метрика, ставшая целью, перестаёт измерять реальность — это классическая ловушка закона Гудхарта, и именно она превращает SLO и SLA в токсичный KPI.

Выход — мерить команду не **абсолютом** SLO, а **дельтой и охватом**:

- не «какой у продуктов SLO», а «на сколько продуктов мы за квартал распространили подписанный SLO и error budget policy»;
- не «выполнен ли SLO», а «сократилось ли сгорание бюджета и money loss»;
- не «сколько девяток», а «вырос ли охват продуктов зрелым диалогом» (слой 3).

Разница тонкая, но критичная. Абсолют SLO — это про продукт, и он принадлежит продакту. KPI команды SRE — это про **то, насколько она продвинула надёжность и диалог о ней**, а это дельта и охват, которые саботировать занижением таргета не выйдет.
