---
title: 'Кластеризация и доступность'
...
Кластеризация БД это один или несколько хостов базы данных, между которыми можно настроить репликацию. Я уже писал отдельно в статье [[MySQL] Репликация Master-Slave](/posts/master-slave/) про репликацию в режиме "master-slave". Поясню "а нахрена оно тебе надо?" Не смотря на очевидность некоторых моментов, не указать их будет глупо.
...
Дополнительно, кластеризация позволяет сильно отсрочить появление ошибки [[MySQL] Максимальное количество коннектов](/posts/max-connections/) и увеличить количество обработанных запросов на чтение (и в некоторых случаях на запись при другом типе репликации).
...
Практическая часть по слоям: [балансировка бэкендов на nginx](/posts/nginx-lb/) и [репликация Master-Slave](/posts/master-slave/) для базы. А как договариваться о том, сколько именно доступности нужно бизнесу, — в [«Надёжность строится в диалоге с бизнесом»](/posts/reliability-is-a-conversation/). Резервирование узлов при этом закрывает только отказ узла: набор правил, по которым сервис переживает отказ зависимости и перегрузку, — [в листе про resilience patterns](https://jtprogru.github.io/The-Way-of-SRE/engineering/resilience-patterns/).
