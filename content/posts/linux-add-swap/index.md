...
По соседним темам: параметр `vm.swappiness` живёт рядом с остальным тюнингом в [«Настройка Linux для HL и защиты от DDoS»](/posts/sysctl-hl/), а если место кончилось не под swap, а под данные — [добавляем диск в LVM без перезагрузки](/posts/add-disk-lvm/).
