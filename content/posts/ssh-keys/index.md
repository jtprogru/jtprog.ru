...
Ручная настройка доступа по ssh-ключу выглядит очень просто – видно из текста выше. Ключи — один из первых пунктов в [моём чек-листе для Linux-сервера](/posts/linux-checklist/), а как красиво выстрелить себе в ногу настройками `ssh_config` в связке с Ansible — [в заметке на полях](/posts/notes-20200925/). На этом всё!
