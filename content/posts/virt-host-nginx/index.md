...
> ⚠️ **CentOS 7 — EOL с 30 июня 2024.** Пост оставлен как есть для тех, кто работает с легаси-серверами. На **Rocky Linux 9** / **AlmaLinux 9** структура `server`-блоков Nginx идентична, меняются только пути SELinux-политик и сам package manager (`yum` → `dnf`). На Debian/Ubuntu файлы конфигов лежат в `/etc/nginx/sites-available/` + симлинки в `sites-enabled/` — на CentOS такой конвенции исторически не было, и многие админы её повторяют вручную.
...
Дальше по теме: [балансировка бэкендов](/posts/nginx-lb/) через `upstream` и [логи nginx в JSON](/posts/nginx-json-logs/) — второе стоит настроить сразу, пока хостов немного.
