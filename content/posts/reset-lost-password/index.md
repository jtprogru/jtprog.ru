...
Далее ждём некоторое время, пока не появится меню recovery mode. Выбираем пункт `root Drop to root shell prompt` и оказываемся в системе под пользователем `root`. Теперь важный момент: как только перед вами откроется консоль сервера, файловая система будет смонтирована в режиме ***только для чтения*** и её необходимо будет перемонтировать для того, чтобы изменения могли сохраниться (вы ничего не сможете создать или изменить). Делается это следующей командой:
...
Если забыт не root системы, а root базы — [сброс пароля MySQL](/posts/reset-root-pass-mysql/). А чтобы не возвращаться к этой статье, настрой [вход по ssh-ключу](/posts/ssh-keys/).
