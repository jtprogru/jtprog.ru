...
Привет, `%username%`! Какое-то время назад я делал опрос на тему [“Должен ли Ops уметь программировать?”](https://t.me/jtprogru_channel/4005) – большинство сошлось во мнении, что “ДА, Ops должен уметь кодить”. И вот теперь коротко о том, какой путь выбрал я.
...
**За годы, что прошли с момента написания этого поста, та самая история «решаю алгоритмы, чтобы не позориться на live-coding» выросла в отдельный проект.** Тогда я тренировался по чужим листам задач и складывал решения в [interview-task](https://github.com/jtprogru/interview-task), а потом собрал всё это в [dsa-for-ops](https://jtprogru.github.io/dsa-for-ops/).

Это структуры данных и алгоритмы именно под эксплуатацию – не «реши 100 задач ради ачивки», а зачем это вообще Ops'у и где оно реально стреляет в работе. Если ты сейчас примерно там же, где был я в 2022-м – вроде кодишь, но на алгоритмах плывёшь – начинай оттуда. Так понятнее, чем с холодного LeetCode.

## Итоги

Итоги подводить пока рано, потому что каких-либо внятных (по моим меркам внятных) достижений еще нет. По моим меркам "внятное достижение" – это "написать свой сервис". Не "микросервис", а полноценную взрослую штуку. Например сервис отложенных постов для своего [Telegram-канала](https://t.me/+LFsXS2FuZiwwMzky), или еще что-то подобное.

Чем это закончилось через несколько лет — в [«Хочу программировать на Golang»](/posts/want-coding-golang/): не сервисом, а кучей мелких утилит под свои задачи. А зачем всё это в контексте роста — в [«Я прошел 50 собесов и вот что я понял»](/posts/after-50-interview/).
