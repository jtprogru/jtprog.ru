...
Теперь при следующем перезапуске MySQL данный параметр будет считан из конфигурационного файла. Заодно посмотри лимиты уровня системы в [«Настройка Linux для HL и защиты от DDoS»](/posts/sysctl-hl/) — упереться в них можно раньше, чем в `max_connections`. На этом всё! Profit!
