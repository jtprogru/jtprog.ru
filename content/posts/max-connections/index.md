...
Теперь при следующем перезапуске MySQL данный параметр будет считан из конфигурационного файла. Заодно посмотри лимиты уровня системы в [«Настройка Linux для HL и защиты от DDoS»](/posts/sysctl-hl/) — упереться в них можно раньше, чем в `max_connections`. А чтобы `500` бралось не с потолка, а из прогноза нагрузки и точки насыщения, — [посмотри лист про capacity planning](https://jtprogru.github.io/The-Way-of-SRE/engineering/capacity-planning/). На этом всё! Profit!
