...
Файрвол в системе linux контролируется программой `iptables` (для ipv4) и `ip6tables` (для ipv6). В данной шпаргалке я указал самые распространённые варианты использования iptables. Знак `#` означает, что команда выполняется от root'а. Получение прав root'а в системе Ubuntu делается командой `sudo -s`, в других системах командой `su`. А в какой именно момент пути пакета отрабатывают эти правила – смотри в разборе [«Что происходит, когда ты открываешь сайт»](/posts/what-happens-when-you-open-website/).
...
По соседним темам: [сетевой тюнинг ядра под нагрузку](/posts/sysctl-hl/), [шпаргалка про маршрутизацию в Ubuntu](/posts/ubuntu-route-add/) и [разбор пути запроса целиком](/posts/what-happens-when-you-open-website/) — там видно, где в этом пути стоит firewall.
