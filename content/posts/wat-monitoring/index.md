...
На рынке уже давно устоялись такие замечательные инструементы как [Zabbix](https://zabbix.com), [Prometheus](https://prometheus.io), [Nagios](https://www.nagios.org/), [OpenNMS](https://www.opennms.org/en), [Cacti](https://www.cacti.net/), [Netdata](https://github.com/netdata/netdata). Обзор данных продуктов я делать не буду, т.к. этой информации предостаточно на просторах паутины — но если хочется сразу пощупать руками, у меня есть пошаговые заметки про [установку Zabbix 5.0](/posts/install-zabbix50/) и [Grafana 7](/posts/install-grafana7/). Главное что необходимо знать о системах мониторинга, это то как они работают (в общих чертах).
...
Этаж выше этой лестницы — [мониторинг бизнес-метрик](/posts/business-metrics/): когда графики зелёные, а оплаты не проходят, инфраструктурный мониторинг молчит.
...
- **Continuous profiling** — постоянный сбор CPU/heap-профилей в проде, чтобы видеть, что грузит сервис прямо сейчас. Инструменты: [Pyroscope](https://pyroscope.io/) (в составе Grafana Labs), [Parca](https://www.parca.dev/), [Polar Signals](https://www.polarsignals.com/).
...
Главная история последних лет в наблюдаемости — это [**OpenTelemetry**](https://opentelemetry.io/) (OTel). Vendor-agnostic стандарт для сбора и передачи метрик, логов и трейсов. Универсальный SDK для приложений и универсальный коллектор (`otel-collector`), который умеет принимать данные в OTLP-формате и отправлять куда угодно: Prometheus-совместимый стор, Tempo/Jaeger, Loki, S3, vendor SaaS.
...
- **[Pixie](https://px.dev/)** — мгновенная observability для Kubernetes без инструментирования;
- **[Cilium Hubble](https://github.com/cilium/hubble)** — observability сетевого трафика в k8s;
- **[Parca](https://www.parca.dev/)** — continuous profiling через eBPF;
- **[Coroot](https://coroot.com/)** — APM без агентов в приложении.
...
| Сигнал | Инструмент |
|--------|-----------|
| Метрики | [VictoriaMetrics](https://victoriametrics.com/) или [Mimir](https://grafana.com/oss/mimir/) (Prometheus-compatible, экономнее по ресурсам) |
| Логи | [Loki](https://grafana.com/oss/loki/) |
| Трейсы | [Tempo](https://grafana.com/oss/tempo/) или [Jaeger](https://www.jaegertracing.io/) |
| Профили | [Pyroscope](https://pyroscope.io/) |
| Коллектор | [Grafana Alloy](https://grafana.com/oss/alloy/) или [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) |
