...
cover:
  ...
  relative: false
type: post
slug: 'sre-recipes-summary'
aliases:
  - '/sre-recipes-summary/'
  - '/2026-02-14-sre-recipes-summary/'
---
...
- Если надежность сервиса нельзя управлять, измерять и регулировать — у тебя нет сервиса, у тебя набор случайно работающих скриптов. Измеряемость начинается с [SLI и SLO, выведенных из пользовательского пути](/posts/slo-as-architecture-blueprint/), а управляемость — с [разговора о них с бизнесом](/posts/reliability-is-a-conversation/).
...
Характерная мысль: надежность — это не героические ночные починки, а заранее сформулированные правила, по которым команда принимает решения, особенно неприятные. Ровно об этом, только на языке производственного цеха, — [«Проект Феникс»](/posts/project-phoenix/): там незапланированная работа съедает команду именно потому, что нет договорённостей, по которым её отсекают.
...
- Всё, что с системой случается, должно попадать в общую память: DIS ([Digital Immune System](/posts/digital-immune-system/), назови как хочешь) — не украшение, а база для эволюции. Чем зрелый иммунитет отличается от набора купленных практик — [разбирал отдельно](/posts/digital-immune-system-maturity/).
...
- Если поверх перцентилей у тебя живут SLO, то алерты вешаются на burn rate. Только помни, что [это безразмерный коэффициент, а не «скорость»](/posts/burn-rate-is-not-speed/), и на маленьком трафике он врёт.
...
По хорошему, у тебя должны мониториться: сами сервисы, их зависимости, мониторинг, срок годности сертификатов/лицензий и [всё, что когда‑нибудь «протухнет»](/posts/wat-monitoring/).
...
Как это выглядит вживую, хорошо было слышно в докладе Mindbox [на Yandex Scale](/posts/yandex-scale-2024/): описания экспериментов лежат рядом с кодом сервиса, на каждый эксперимент готов runbook по восстановлению, а окно для запуска — середина нагрузки, а не пик и не ночь.
...
Как это раскладывается по слоям — что резервируем на фронте, что на бэке, а что в базе — [разбирал в посте про кластеризацию и доступность](/posts/ha-clustering/).
...
По сути, это чистый FinOps: договариваешься с бизнесом, какой трафик обязан выдерживать, а какой — нет, и уже под это строишь схему резервирования и бюджетов. И это не разовая мода — [за 2020-2025 деньги окончательно въехали в зону ответственности SRE](/posts/sre-evolution-2020-2025/) вместе с Platform Engineering.
...
- Всё, что имеет ценность и выглядит как файл, должно жить в VCS: скрипты, конфиги, инфраструктурные артефакты. Дашборды Grafana сюда тоже входят — их проще [выгрузить через API и положить в provisioning](/posts/grafana-exports/), чем перекликивать мышкой после переезда.
...
- Инструкция должна быть такова, чтобы её могла выполнить «Людочка» или условная секретарша: копировать команды, открывать ссылки и получать предсказуемый результат. Писать её стоит как код, с тем же ревью и версиями — [про RTFM, DRY и документацию у меня есть отдельный текст](/posts/rtfm-dry-kiss/).
- Плановые работы должны делаться **по регламенту**, без «давай заодно тут поправим, пока уже всё отключили».

Сюда же — план Б на каждое нетипичное вмешательство: если «всё пойдёт не так», должен быть понятный сценарий возврата в состояние «как было». Предел мечтаний тут — инструкция, которую вообще не надо читать руками: [Tron из одноимённого фильма](/posts/tron-as-sre/) как раз про автоматический runbook remediation, только с мечом.
...
Отдельная история — групповые дежурства: если «ответственность на всех», то по факту не отвечает никто, поэтому нужны конкретные фамилии в графике. Как делить зоны ответственности внутри команды, чтобы график не был формальностью, [писал ещё во времена сисадминских будней](/posts/organize-sysadmins/).
...
Плюс базовая дисциплина: общий календарь маркетинговых запусков и регламентных работ, понятные термины и общий словарь. Это и есть та часть [DevOps](/posts/wat-devops/), которая про совместную работу команд, а не про очередной пайплайн.
...
ИБ — просто ещё один класс рисков для надёжности, и игнорировать его — то же самое, что не мониторить диск или БД. Как это выглядит, когда риск реализуется целиком, хорошо видно на [разборе истории с «Аэрофлотом»](/posts/hacking-aeroflot/).
...
- **Наведи порядок в мониторинге.** Абсолютные значения + тренды, min/max + перцентили, резервный мониторинг и внешние проверки. И добавь сверху [бизнес-метрики](/posts/business-metrics/) — без них ты видишь живые хосты, но не видишь, что оплата не проходит.
- **Инвестируй в процесс.** Стандартизированные инструкции, регламенты, планы отката, регулярные учения по релизам и rollback’ам.
- **Живи через GitOps и версии.** Всё важное — в VCS, релиз самодостаточен, откат — быстрый и предсказуемый. [Лист про GitOps](https://jtprogru.github.io/The-Way-of-SRE/engineering/gitops/) — про то, чем модель pull отличается от «задеплоил из пайплайна».
- **Не забывай про людей.** Психология дежурств, личные чек‑листы, явные дежурные, нормальная коммуникация с продактами и смежниками. [Про устройство ротации](https://jtprogru.github.io/The-Way-of-SRE/practices/on-call-rotation/) — покрытие, справедливость и выносливость — есть отдельный лист.
