...
Знать и работать прям со всеми системами инициализации тебе вряд ли придется, но знать об их наличии я считаю надо. В большинстве систем с которыми приходится работать, обычно используется `systemd` — а с ним рядом сразу оказываются [`systemd-resolved`](/posts/systemd-resolved/) для DNS и [`systemd-timesyncd`](/posts/systemd-timesyncd/) для времени.
