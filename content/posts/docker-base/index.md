...
Согласно одной из легенд, `docker-compose` появился после того, как к разработчикам Docker пришли и сказали: "Docker – отличная вещь! Но сделайте удобно!" Будем считать что он уже [поставлен](/posts/docker-install/) у вас. Так что сразу перейдем к делу – содаем локально рядом с `Dockerfile` еще и `compose.yml` (или `docker-compose.yml` — старое имя файла, его до сих пор подхватывают все версии compose).
...
Как быть дальше и с какой стороны/силы приложить напильник, будет описано в следующей статье. Из практического рядом: [как зайти в работающий контейнер и выполнить команду внутри](/posts/edit-file-in-docker/) — пригодится в первый же вечер отладки. А живой пример compose-стека целиком — [установка Netbox через netbox-docker](/posts/netbox/). Куда всё это растёт дальше — планировщик, health-чеки, лимиты и остальное, что надо знать про кластер помимо `kubectl`, — [в листе про контейнеры и оркестрацию](https://jtprogru.github.io/The-Way-of-SRE/engineering/container-orchestration/).
