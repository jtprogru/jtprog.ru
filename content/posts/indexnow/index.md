...
Дальше — как закрыть эту боль одним шагом в GitHub Actions через мой CLI [indexnow](https://github.com/jtprogru/indexnow) и почему оно вообще работает. Как устроен сам пайплайн блога, я рассказывал в [«Github Actions для блогера»](/posts/github-actions/) — новый шаг встаёт туда же, после деплоя.
...
Поэтому я написал на [Go](/posts/want-coding-golang/) маленький `indexnow` — сначала как CLI для разовых submission'ов, а с релиза `v0.5.0` это уже и **готовый GitHub Action**. По дороге всё то, что обычно копится в самописных скриптах, я аккуратно унёс внутрь бинаря.
