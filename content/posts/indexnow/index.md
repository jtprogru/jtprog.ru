...
lastmod: 2026-06-02T18:00:00+03:00
tags:
  - hugo
  - github-actions
  - seo
...
Привет, `%username%`! Знакомая боль: выкатил новый пост, поправил страницу, обновил title — и сидишь ждёшь, пока поисковики соизволят переобойти сайт. День, два, неделя. А хочется, чтобы апдейт подхватился сразу после `git push`, без танцев с Search Console и ручных submission-форм.
...
Заодно это стало хорошим pet-project'ом: тесты с `-race`, `golangci-lint`, `mkdocs` для документации, релизы через `goreleaser`, а теперь и composite Action с верификацией бинарей. Маленький инструмент, который реально экономит время и заодно держит руки в форме.
...
Ключ передаётся через `env`, а не флагом, — даже если в твоём шаге случайно отрабатывает `set -x`, ключ в логах не светится. В `$GITHUB_STEP_SUMMARY` после прогона падает короткий отчёт: сколько URL улетело, сколько батчей упало, какой эндпоинт использовался. Не нужно лезть в полный лог раннера, чтобы понять, прошёл submit или нет.
...
Это и есть нужный мне маппинг: `content/<slug>/index.md` → `https://jtprog.ru/<slug>/`. Никакого парсинга sitemap, никаких лишних submission'ов на страницы, которые не трогал. `fetch-depth: 0` обязательно — без полной истории `git diff` против `github.event.before` молча отдаст пустоту.
