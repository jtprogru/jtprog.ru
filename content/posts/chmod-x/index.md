...
> **🔄 Обновлено 2026-05-29.** Добавил в начало блок с быстрым ответом про `chmod +x`/`a-x` для тех, кто забрёл сюда за базой, и починил пару примеров в коде (perl, Go).

## Быстрый ответ: chmod +x, a+x, a-x

Если ты пришёл сюда просто за тем, как сделать файл исполняемым — держи, без воды:

- `chmod +x file` — добавить бит исполнения (с учётом `umask`, обычно владельцу, группе и остальным);
- `chmod a+x file` — добавить бит исполнения **всем** (`a` = `user` + `group` + `other`);
- `chmod a-x file` — наоборот, **снять** бит исполнения у всех;
- `chmod u+x file` — только владельцу, `g+x` — группе, `o+x` — остальным.

Тут `x` — это execute, `r` — read, `w` — write. Плюс добавляет бит, минус снимает. Вот и вся базовая магия.

Это азы. А дальше — про куда более весёлый случай: что будет, если этим же `chmod` снять бит исполнения у самого бинарника `chmod`, и как из этой ямы выбираться.
...
Да, этот способ – это очередное подтверждение того, что ops должен уметь кодить (хотя бы на уровне говнокода) – о том, почему это надо более подробно я [упоминал тут](/posts/dev-and-work/).
...
Выполнение в консоли `go run chmodfixer.go` просто запустит данный "скрипт", а `go build -o /tmp/chmodfixer chmodfixer.go` скомпилирует данный код, а исполняемый файл сохранит в `/tmp/chmodfixer`.
...
Основная задача данного вопроса состоит не в количестве придуманных вариантов решения, а в проверке хода мышления в самой нестандартной ситуации при решении проблемы, с которой точно не сталкивался человек. Из той же серии — [задачка с собеса 0003](/posts/interview-task-0003/), где тоже проверяют не факты, а рассуждение.

А если пришёл сюда за практикой, а не за собесом: [рекурсивный chmod через `find`](/posts/recursive-chmod/) — то, что реально нужно в 99% случаев.
