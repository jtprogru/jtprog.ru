...
Привет, `%username%`! Данная статья - обычный пересказ официальной документации [Zabbix](https://www.zabbix.com/download?zabbix=5.0&os_distribution=ubuntu&os_version=20.04_focal&db=postgresql&ws=nginx) и не содержит в себе ничего магического. Да и процесс установки уже мною освещался ранее на [CentOS](https://jtprog.ru/posts/install-zabbix-centos/) и [Ubuntu](https://jtprog.ru/posts/install-zabbix-ubuntu/).
...
Теперь необходимо настроить авторизацию по LDAP – но там всё очень просто. Дальше логично прикрутить сверху [Grafana](/posts/install-grafana7/) для графиков и почитать [«Мониторинг: что/куда/зачем?»](/posts/wat-monitoring/) — про то, что и зачем вообще собирать. На это всё!
