...
aliases:
  ...
  - '/2026-01-05-sre-evolution-2020-2025/'
---
...
Считать toil при этом надо раньше, чем автоматизировать: без классификации и замера автоматизируется то, что громче болит, а не то, что дороже обходится. [Про учёт toil](https://jtprogru.github.io/The-Way-of-SRE/engineering/toil-tracking/) — отдельный лист в карте компетенций.
...
Что при этом отличает платформу от кучи скриптов с общим README — границы, интерфейсы и измерение спроса — [в листе про platform engineering](https://jtprogru.github.io/The-Way-of-SRE/engineering/platform-engineering/).
...
Catchpoint 2025 говорит, что Capacity Management теперь в приоритете у компаний, наравне с SRE, SLO и автоматизацией починки инцидентов. Как эта ответственность выглядит инженерно, а не как отчётность для финансов, — [в листе про управление стоимостью облака](https://jtprogru.github.io/The-Way-of-SRE/engineering/cloud-cost-control/).
...
*  Надо думать о пользователях: SLO должны показывать, что пользователи чувствуют, а не только технические метрики. Как выводить SLI из критичного пользовательского пути, я разобрал в [«SLO как чертёж архитектуры»](/posts/slo-as-architecture-blueprint/), а про [сгорание бюджета ошибок](/posts/burn-rate-is-not-speed/) — отдельно, потому что burn rate почти все понимают неверно.
...
2.  **Observability – это важно**: переходите от [простого мониторинга](/posts/wat-monitoring/) к полноценной observability с OpenTelemetry, а сверху добавляйте [бизнес-метрики](/posts/business-metrics/).
3.  **SLO – это культура**: SLO должны быть не просто цифрами, а [частью разговора с бизнесом](/posts/reliability-is-a-conversation/) и процесса принятия решений.
4.  **Пробуйте Chaos Engineering**: начните с маленьких экспериментов в staging и постепенно переходите к production — это один из столпов [Digital Immune System](/posts/digital-immune-system/).
...
Если после этого списка хочется не «что почитать», а «что уметь и в каком порядке», — я как раз собираю такую карту: [The Way of SRE](https://jtprogru.github.io/The-Way-of-SRE/priorities/), три ветви развития и приоритет по каждой компетенции.
