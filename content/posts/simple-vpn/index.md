...
> ⛔ **Не поднимай PPTP в 2026 году.** Протокол сломан криптографически, а не «морально устарел». Аутентификация MS-CHAPv2 сводится к одному перебору DES-ключа: сервис [chapcrack + CloudCracker](https://www.cloudcracker.com/blog/2012/07/29/cracking-ms-chap-v2/) ещё в 2012 году показал восстановление ключа за сутки на FPGA-ферме, сейчас это дешевле и быстрее. Шифрование MPPE построено на RC4 и не защищает целостность — трафик не только читается, но и модифицируется. Microsoft [сама рекомендует уходить с PPTP](https://learn.microsoft.com/en-us/windows-server/remote/remote-access/), а в Windows Server 2025 поддержка PPTP и L2TP в RRAS объявлена устаревшей.
>
> Что использовать вместо: **WireGuard** — если нужен просто быстрый туннель между своими устройствами, конфиг умещается в десять строк. **OpenVPN** — если нужна аутентификация по сертификатам, сложная маршрутизация или прохождение через недружелюбные сети по TCP/443. Оба поддерживаются всеми клиентами, ради которых в 2015-м и брался PPTP: и Android, и Windows, и iOS умеют их из коробки или одним приложением.
>
> Пост оставлен как есть — как исторический документ и на случай, если тебе достался в наследство работающий pptpd и надо понять, что там вообще настроено. Разворачивать по нему новый сервер не надо.
