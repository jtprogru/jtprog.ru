...
Теперь нам надо установить БД PostgreSQL 12 (она идет в дефолтных репозиториях и меня устраивает), но делать мы этого не будем – на данном сервере у меня развернут [Zabbix 5.0](/posts/install-zabbix50/), к которому я и установил PostgreSQL. Мы же рассматрим несколько простых команд, которые необходимо выполнить для того, чтобы подружить PostgreSQL и Grafana:
...
После чего идем в браузер и открываем адрес `https://graf.jtprog.ru` – короче тот, который вы настроили у себя. Логинимся в Grafana с дефолтными логином и паролем `admin`/`admin` и сразу же меняем пароль на что-то посложнее и [сгенерированное в консоли](/posts/cli-gen-pass/).
