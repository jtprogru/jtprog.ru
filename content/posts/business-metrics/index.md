...
Привет, `%username%`! Сегодня поговорим о теме, которая часто остается в тени инфраструктурного мониторинга, но при этом не менее важна — о **мониторинге бизнес-метрик**. Про [мониторинг вообще — что, куда и зачем собирать](/posts/wat-monitoring/) — у меня есть отдельный текст, а тут речь про слой выше. Если ты SRE, DevOps или TeamLead, то наверняка сталкивался с ситуацией: все графики зеленые, CPU не плавится, память в норме, а бизнес приходит и говорит "что-то не работает". И вот тут начинается самое интересное.
...
**Бизнес-слой** — это то, что видит конечный пользователь. Для платежной системы это объемы успешных авторизационных запросов, количество операций по типам, время обработки платежей. Для интернет-магазина — это количество успешных заказов, средний чек, конверсия в покупку. Для SaaS-продукта — количество активных подписок, revenue per user, churn rate. Строгая версия этого слоя — [SLI и SLO, выведенные из критичного пользовательского пути](/posts/slo-as-architecture-blueprint/): те же «успешные заказы», только с порогом, окном и бюджетом ошибок.
...
**Grafana** — универсальная обертка для визуализации ([как её поставить](/posts/install-grafana7/) — на пару абзацев дела). Она сама ничего не собирает и не хранит, но умеет красиво отображать данные из Prometheus, PostgreSQL, ClickHouse и еще десятков источников.
...
Разница между этими двумя строчками — это [дисциплина «симптом, а не причина»](https://jtprogru.github.io/The-Way-of-SRE/engineering/symptom-vs-cause-alerting/): будим человека на то, что чувствует пользователь, а метрики причин оставляем для отладки. А если шума уже накопилось столько, что дежурный смотрит мимо, — [про разгребание усталости от алертов](https://jtprogru.github.io/The-Way-of-SRE/engineering/alert-fatigue-management/) есть отдельный лист.
...
5. **Делай метрики основой для принятия решений**: не просто смотри на графики, а делай выводы и действуй — как именно этот разговор ведётся с бизнесом, разбирал в [«Надёжность строится в диалоге с бизнесом»](/posts/reliability-is-a-conversation/)
