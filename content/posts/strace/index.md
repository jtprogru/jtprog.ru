...
> 🔄 **Обновлено 2026-05-15**: `strace` по-прежнему живой и нужный инструмент — содержание поста не меняю. В конце добавил раздел «Когда `strace` мало»: что использовать, когда overhead слишком большой или нужна не системные вызовы, а ядро/сеть/диск целиком. Спойлер: `bpftrace`, `perf trace`, `bcc-tools`.
...
У `strace` есть один большой и неприятный недостаток — он работает через `ptrace(2)`, что **остро замедляет процесс**: типичный overhead х10-х100 на каждый системный вызов. На лайв-проде это часто либо роняет SLO, либо вообще делает наблюдение бесполезным (timing полностью меняется). А ещё `strace` не покажет тебе ничего, что происходит **внутри** ядра — только границу userspace ↔ kernel.

В 2026-м для таких задач есть набор eBPF-инструментов, которые работают почти без overhead и видят сильно больше. Не вместо `strace` (для быстрой диагностики «куда write провалился» он по-прежнему незаменим), а в дополнение.

### perf trace

Что-то вроде `strace`, но через perf-events. Overhead заметно ниже, чем у `ptrace`-based strace:

```bash
sudo perf trace -p <PID>
sudo perf trace -e openat,read,write -p <PID>
sudo perf trace --duration 10 nginx -t  # трейс с фильтром «дольше 10 мс»
```

Полезно когда `strace` слишком медленный и при этом нужны именно системные вызовы.

### bpftrace

[bpftrace](https://github.com/bpftrace/bpftrace) — DSL поверх eBPF, чем-то напоминает DTrace из соларки. Можно писать собственные пробы быстро, без компиляции и без боли:

```bash
# Какие файлы открывает процесс с PID 1234
sudo bpftrace -e 'tracepoint:syscalls:sys_enter_openat /pid == 1234/
    { printf("%s\n", str(args->filename)); }'

# Гистограмма времени чтения с диска по всему хосту
sudo bpftrace -e 'kprobe:vfs_read { @start[tid] = nsecs; }
                  kretprobe:vfs_read /@start[tid]/
                    { @ns = hist(nsecs - @start[tid]); delete(@start[tid]); }'
```

Это уже не «что вызвал процесс», а «сколько ядро потратило на каждое чтение» — в данных гистограмма по латенси, видно длинные хвосты.

### bcc-tools

[bcc](https://github.com/iovisor/bcc) — старший брат `bpftrace`, набор готовых утилит. Если не хочется писать свои пробы — берёшь готовое:

| Утилита | Что показывает |
|---------|----------------|
| `opensnoop-bpfcc` | Кто открывает файлы (с задержками) |
| `execsnoop-bpfcc` | Кто запускает новые процессы |
| `tcpconnect-bpfcc` | Кто инициирует TCP-коннекты |
| `tcpretrans-bpfcc` | Ретрансмиты TCP в реальном времени |
| `biolatency-bpfcc` | Гистограмма latency блочных I/O |
| `runqlat-bpfcc` | Сколько процессы ждут CPU в run queue |
| `funccount-bpfcc` | Счётчик вызовов функции ядра |

Всё это без модификации приложения, без перезапуска, с минимальным overhead. Если на проде шторм latency и не понятно где — `biolatency-bpfcc` + `runqlat-bpfcc` за минуту дадут понять, диск тормозит или планировщик.

### sysdig / falco

Для security-сценариев и audit'а ещё стоит знать про [sysdig](https://github.com/draios/sysdig) и [Falco](https://falco.org/) — они работают на тех же eBPF/kprobe'ах, но заточены не на debugging, а на «у нас тут шелл из контейнера, бейся в SIEM».

### Шпаргалка «что когда брать»

| Задача | Инструмент |
|--------|-----------|
| Быстро посмотреть, что делает один процесс | `strace` |
| То же, но прод и overhead критичен | `perf trace` |
| Свой кастомный замер по событиям ядра | `bpftrace` |
| Готовые «снупы» по конкретным подсистемам (FS/net/sched) | `bcc-tools` |
| Long-running audit с экспортом в SIEM | `sysdig` / `falco` |

## Резюме

`strace` может оказаться (и оказывается!) весьма полезной утилитой как для программистов, так и для системных администраторов, помогая отыскать причины "падений" или некорректной работы программ с закрытыми или недоступными исходными кодами. Когда его становится мало или он слишком тяжёлый — рядом всегда есть `perf trace`, `bpftrace` и `bcc-tools`. А чтобы понимать, на каком этаже вообще стоит доставать `strace`, а на каком проблема лежит выше – держи под рукой [карту пути запроса](/posts/what-happens-when-you-open-website/). И если хочется не отдельную утилиту, а рамку — что вообще SRE стоит знать про ОС и в каком порядке это набирать, — [есть лист в карте компетенций](https://jtprogru.github.io/The-Way-of-SRE/engineering/operating-systems/).
