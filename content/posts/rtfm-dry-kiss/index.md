---
title: 'Документация, Сухой поцелуй и джуны'
...
keywords: ["документация в IT", "rtfm что значит", "dry принцип", "kiss принцип", "обучение джунов", "советы для junior", "самообучение в IT", "менторство в IT", "работает не трогай", "docs-as-code", "diataxis", "llm и документация"]
...
> 🔄 **Обновлено 2026-05-15**: основной текст про RTFM/DRY/KISS и менторство не трогаю — это всё актуально. В конец добавил короткий блок «Документация в эпоху LLM»: как изменилось чтение/написание доков за пять лет, что в них хорошо ищется, что галлюцинируется, и как подход Diátaxis и docs-as-code помогают делать доки, которые не разваливаются.
...
В общем тебе надо очень быстро впитать в себя довольно большой объём информации (да – я вернулся к тому с чего начинал). И если снова взглянуть на пример выше, то ты для актуализации своих знаний читаешь доку – RTFM! А благодаря ведению каких-то заметок ты начинаешь формировать документацию. Когда-то давно этот блог задумывался именно как заметочник, чтоб не искать нужные куски документации при решении каких-то проблем. Со временем заметки переехали в [персональную базу знаний](/posts/pkb-check-list/) — там про то, как их складывать, чтобы потом находить.
...
## Документация в эпоху LLM

Когда я писал этот пост в 2021-м, основной мой тезис был: «учись читать документацию и правильно гуглить». В 2026-м к «гуглить» добавился ещё один важный навык — **разговаривать с LLM**. И это не отменяет первый, а дополняет.

### Что LLM делает с документацией хорошо

- **Навигация в большом объёме**. Раньше чтобы найти, как настроить TLS-handshake в HAProxy 3.x, ты читал 80-страничную doc-страницу и искал по `Ctrl-F`. Сейчас спросил «как в HAProxy включить TLS с client cert auth» — получил рабочий конфиг с пояснением. Не надо забывать сверяться с актуальной официальной документацией (LLM может цитировать устаревшее).
- **Перевод между диалектами**. «Покажи как этот ansible-плейбук переписать на Terraform», «вот пример Compose v2 на v3», «как этот SQL на PostgreSQL запустить в MySQL». В 80% случаев работает.
- **Стартовый шаблон документации**. Дать LLM описание команды/API и попросить «напиши docstring/README» — это лучше, чем чистая страница. Сэкономит 30-50% времени.

### Что LLM делает плохо

- **Галлюцинирует имена флагов, путь конфигов, версии**. Особенно для нишевых тулов. Если кто-то говорит «я просто спросил у Claude/GPT и сделал по их инструкции» — это валидное начало, но не валидный результат: всегда сверяйся с `--help`, `man`, официальной docs-страницей.
- **Выдаёт устаревший синтаксис**. Модель обучена на корпусе, который замирает в определённый момент времени. Compose v1, Python 2.x, удалённые из API эндпойнты — она выдаст уверенно.
- **Не знает контекст твоего проекта**. Если у вас в проекте кастомный стиль, форки, нестандартные конвенции — LLM их не знает. Этот пробел закрывают [MCP-серверы](https://modelcontextprotocol.io/) и инструменты типа Claude Code/Cursor, которые читают репозиторий — но это уже отдельный навык.

### Diátaxis: четыре типа документации

Параллельно с появлением LLM в индустрии прижился фреймворк [**Diátaxis**](https://diataxis.fr/). Идея простая — любая документация делится на четыре типа, и не надо их смешивать:

| Тип | Цель | Аналогия |
|-----|------|----------|
| **Tutorial** | Обучение через действие | Урок |
| **How-to guide** | Решение конкретной задачи | Рецепт |
| **Reference** | Точные технические сведения | Энциклопедия |
| **Explanation** | Понимание идей и контекста | Лекция |

Главная польза для тех, кто пишет доки: **перестань делать FAQ-страницу с двадцатью разнотипными вопросами вперемешку**. Раздели на четыре раздела — читать станет в разы проще, и LLM по такой структуре отвечает заметно точнее.

### Docs-as-code

Это уже не новость, но за пять лет docs-as-code из «модного слова» окончательно стало нормой. Doc'и живут в Markdown в том же репо, что и код, ревьюятся через PR, рендерятся через MkDocs/Docusaurus/Hugo. Польза:

- доки версионируются вместе с кодом;
- PR с фичей и PR с доками — это один PR;
- легко прогонять через линтер (`vale`, `markdownlint`);
- LLM-инструменты понимают такую структуру лучше, чем Confluence-страницу.

### TL;DR апдейта

Учиться читать документацию по-прежнему обязательно. К этому теперь добавились два навыка: **уметь верифицировать выхлоп LLM** (не верить на слово) и **писать доки так, чтобы их понимал и человек, и модель** — структура Diátaxis, docs-as-code, никаких портянок без заголовков.

Если хочешь, как именно я организую свою личную базу знаний с этим всем учётом — почитай [Содержимое моей персональной базы знаний](/posts/pkb-check-list/). А в эксплуатации у документации есть два жанра, которые стоит различать: [runbook](https://jtprogru.github.io/The-Way-of-SRE/culture/runbooks/) — что нажать при конкретном алерте, и [playbook](https://jtprogru.github.io/The-Way-of-SRE/culture/playbooks/) — кто и что решает, пока нажимают.
