...
> ⚠️ **CentOS 7 — EOL с 30 июня 2024.** Пост оставлен как есть для тех, кто всё ещё поддерживает легаси-серверы. Для новых установок используй **Rocky Linux 9** или **AlmaLinux 9** — команды установки и настройки NFS идентичны (`yum` → `dnf`, `firewalld` и `exports` без изменений). На EL9 также имеет смысл присмотреться к **nfs-ganesha** как user-space альтернативе ядерному `nfsd`.
