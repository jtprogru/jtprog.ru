...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y update \
    ...
    && echo "${SSH_PK}" |  tr -d ' ' | base64 --decode > /root/.ssh/id_rsa && chmod 600 /root/.ssh/id_rsa && cat /root/.ssh/id_rsa \
    && cp ./content/.htaccess ./public/.htaccess && cp ./content/robots.txt ./public/robots.txt \
    && echo "Запуск rsync" && rsync -avz -e "ssh -i ~/.ssh/id_rsa" --progress --delete public/ jtprogru@jtprog.ru:/home/jtprogru/subdomains/hugo \
